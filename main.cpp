...
// The lexer returns tokens [0-255] if it is an unkown character, otherwise one
...
// token the parse is looking at. getNextToken reads another token from the
...
static std::unique_ptr<ExprAST> ParsePrimary() {
    switch (CurTok) {
        default: return LogError("unkown token when expecting an expression");
...
static std::unique_ptr<PrototypeAST> ParsePrototype() {
    if (CurTok != tok_identifier) {
        return LogErrorP("Expcected function name in prototype");
    }
